...
namespace Drn
{
	...
	D3D12_VERTEX_BUFFER_VIEW SkeletalMeshVertexBuffer::VertexBufferViewNull;
...
// --------------------------------------------------------------------------------------------------------------
	...
	SkeletalMeshVertexBuffer::SkeletalMeshVertexBuffer()
		: m_PositionBuffer(nullptr)
		, m_NormalBuffer(nullptr)
		, m_TangentBuffer(nullptr)
		, m_BitTangentBuffer(nullptr)
	...
	SkeletalMeshVertexBuffer* SkeletalMeshVertexBuffer::Create( class D3D12CommandList* CommandList, SkeletalMeshVertexData& Source, const std::string& Name, bool CreateOnDefaultHeap )
	{
		SkeletalMeshVertexBuffer* Result = new SkeletalMeshVertexBuffer;
		...
		CreateVertexBufferConditional(true, CommandList, Result->m_BoneIndices, (void*)Source.GetBoneIndices().data(), Source.GetVertexCount() * sizeof(uint32), "VB_BoneIndices");
		CreateVertexBufferConditional(true, CommandList, Result->m_BoneWeights, (void*)Source.GetBoneWeights().data(), Source.GetVertexCount() * sizeof(uint32), "VB_BoneWeights");

		return Result;
	}

	void SkeletalMeshVertexBuffer::Bind( class D3D12CommandList* CommandList )
	{
		CommandList->SetStreamSource(0, m_PositionBuffer, 0);
		CommandList->SetStreamSource(1, m_ColorBuffer, 0);
		CommandList->SetStreamSource(2, m_NormalBuffer, 0);
		...
		CommandList->SetStreamSource(6, m_UV3Buffer, 0);
		CommandList->SetStreamSource(7, m_UV4Buffer, 0);
		CommandList->SetStreamSource(8, m_BoneIndices, 0);
		CommandList->SetStreamSource(9, m_BoneWeights, 0);
	}
