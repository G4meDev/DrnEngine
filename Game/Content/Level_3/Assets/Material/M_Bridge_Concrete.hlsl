...
{
    SCALAR(DetailNormalStrength, DetailNormalStrength)
    
    TEX2D(BridgeBaseColor, BridgeBaseColorTexture)
    TEX2D(DetailNormal, DetailNormalTexture)
};
...
PixelShaderOutput Main_PS(PixelShaderInput IN) : SV_Target
{
    ...
    Texture2D BridgeBaseColorTexture = ResourceDescriptorHeap[Parameters.BridgeBaseColor_Texture];
    SamplerState BridgeBaseColorSampler = ResourceDescriptorHeap[Parameters.BridgeBaseColor_Sampler];
    
    Texture2D DetailNormalTexture = ResourceDescriptorHeap[Parameters.DetailNormal_Texture];
    SamplerState DetailNormalSampler = ResourceDescriptorHeap[Parameters.DetailNormal_Sampler];
    
    float2 UV0 = IN.UV0;;
    
    float3 VertexNormal = IN.TBN[1];

    float3 BaseColor = BridgeBaseColorTexture.Sample(BridgeBaseColorSampler, UV0).rgb;
    float Roughness = saturate(lerp(0.8f, -0.1f, BaseColor.r));
    BaseColor *= BaseColor;
    
    float3 DetailNormal = ReconstructTextureNormal(DetailNormalTexture.Sample(DetailNormalSampler, UV0).rg, true);
    DetailNormal *= float3(Parameters.DetailNormalStrength, 1, Parameters.DetailNormalStrength);
    
    float3 TangentNormal = DetailNormal;
