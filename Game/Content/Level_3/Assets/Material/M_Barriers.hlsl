...
struct ParametersBuffers
{
    ...
    SCALAR(ColorIntensity, ColorIntensity)
...
PixelShaderOutput Main_PS(PixelShaderInput IN) : SV_Target
{
    ...
    float Roughness = saturate((BaseColor.r + 0.5f) * 0.5f);
    BaseColor *= Parameters.ColorIntensity;
    
    float3 WorldNormal = VertexNormal;
    
    float3 Masks = float3(Parameters.Metallic, Roughness, 1.0f);
