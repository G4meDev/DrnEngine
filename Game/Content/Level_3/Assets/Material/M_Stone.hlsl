...
{
     VECTOR(BaseColorTint, BaseColorTint)
...
PixelShaderOutput Main_PS(PixelShaderInput IN) : SV_Target
{
    ...
    float3 BaseColor = BaseColorTexture.Sample(BaseColorSampler, UV0).rgb;
    BaseColor *= Parameters.BaseColorTint.rgb;
    BaseColor *= MasksTexture.Sample(MasksSampler, UV0).g * 1.5f;
    float Roughness = 0.8f;
    
    float3 TangentNormal = ReconstructTextureNormal(NormalTexture.Sample(NormalSampler, UV0 * 3).rg, false);
