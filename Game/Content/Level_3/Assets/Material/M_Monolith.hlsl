...
struct ParametersBuffers
{
    ...
    TEX2D(OrnamentMasks, OrnamentMasksTexture)
    TEX2D(OrnamentNormal, OrnamentNormalTexture)
};
...
PixelShaderOutput Main_PS(PixelShaderInput IN) : SV_Target
{
    ...
    Texture2D OrnamentMasksTexture = ResourceDescriptorHeap[Parameters.OrnamentMasks_Texture];
    SamplerState OrnamentMasksSampler = ResourceDescriptorHeap[Parameters.OrnamentMasks_Sampler];
    
    Texture2D OrnamentNormalTexture = ResourceDescriptorHeap[Parameters.OrnamentNormal_Texture];
    SamplerState OrnamentNormalSampler = ResourceDescriptorHeap[Parameters.OrnamentNormal_Sampler];
    ...
    float4 MonolithMasks = MasksTexture.Sample(MasksSampler, UV1);
    float4 WallMasks = WallMasksTexture.Sample(WallMasksSampler, UV0);
    float4 WallAO = WallAOTexture.Sample(WallAOSampler, UV0).r;
    float4 OrnamentMasks = OrnamentMasksTexture.Sample(OrnamentMasksSampler, UV2);
    float4 OrnamentNormal = OrnamentNormalTexture.Sample(OrnamentNormalSampler, UV2);
    
    float3 BaseColor = lerp(Parameters.ColorCleanBricks1.rgb, Parameters.ColorCleanBricks2.rgb, MonolithMasks.b);
    BaseColor = lerp(BaseColor, Parameters.ColorPaintedBricks.rgb, MonolithMasks.r);
    BaseColor *= lerp(BaseColorTexture.Sample(BaseColorSampler, UV0).rgb, OrnamentMasks.rgb, OrnamentMasks.a);
    ...
    BaseColor = lerp(BaseColor, Parameters.ColorTip.rgb, MonolithMasks.ggg);
    ...
    Roughness = lerp(Roughness, Parameters.RoughnessPaint, MonolithMasks.r);
    Roughness = lerp(Roughness, Parameters.RoughnessTip, MonolithMasks.g);
    Roughness = saturate(Roughness);
    float3 Masks = float3(MonolithMasks.g, Roughness, 1.0f);
    ...
    float3 OrnoNormal = ReconstructTextureNormal(OrnamentNormal.rg, true);
    OrnoNormal *= float3(4, 1, 4);
    ...
    TangentNormal = lerp(TangentNormal, OrnoNormal, OrnamentMasks.a);
