...
struct ParametersBuffers
{
    ...
    VECTOR(EmssiveColor, EmssiveColor)
    ...
    TEX2D(EmssiveMasks, EmssiveMasksTexture)
};
...
VertexShaderOutput Main_VS(VertexInputStaticMesh IN)
{
    ...
    OUT.Position = mul(Primitive.LocalToWorld, float4(IN.Position, 1.0f));
    ...
    float3 WorldPosition = mul(Primitive.LocalToWorld, float4(IN.Position, 1.0f)).xyz;
    ...
    OUT.Position = mul(Primitive.LocalToProjection, float4(IN.Position, 1.0f));
    OUT.Color = float4(IN.Color, 1.0f);
...
PixelShaderOutput Main_PS(PixelShaderInput IN) : SV_Target
{
    ...
    Texture2D EmssiveTexture = ResourceDescriptorHeap[Parameters.EmssiveMasks_Texture];
    SamplerState EmssiveTextureSampler = ResourceDescriptorHeap[Parameters.EmssiveMasks_Sampler];
    ...
    float3 Emssive = EmssiveTexture.Sample(EmssiveTextureSampler, IN.UV1).xxx;
    Emssive *= Parameters.EmssiveColor.rgb * Parameters.EmssiveColor.a;
    
    OUT.ColorDeferred = float4(Emssive, 1);
